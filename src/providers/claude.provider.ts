...
export class ClaudeProvider extends BaseAIProvider {
  ...
  async generateResponse(request: AIProviderRequest): Promise<AIProviderResponse> {
    ...
    return this.deduplicateRequest(requestKey, async () => {
      ...
      tradingLogger.apiCall('Claude', 'generateResponse', true, 0);
      ...
      try {
        ...
        tradingLogger.apiCall('Claude', 'generateResponse', false, Date.now() - startTime);
        throw error;
      }
    });
  }
...
Remember: The goal is consistent profitability with managed risk, not chasing high-risk, high-reward opportunities.`;
  ...
  // Claude-specific trading methods leveraging its analytical strengths
  async generateTradingSignal(
    ...
    const prompt = this.buildDetailedTradingPrompt(
      symbol,
      marketData,
      riskParams,
      historicalContext
    ...
    return this.generateResponse({
      prompt,
      temperature: 0.2, // Very low temperature for consistent trading analysis
      maxTokens: 1500,
...
Provide a structured analysis with specific actionable insights for trading decisions.`;
  ...
  async generateChainOfThoughtAnalysis(
    problem: string,
    availableData: any[],
    constraints: any[]
  ): Promise<AIProviderResponse> {
...
Provide your analysis in a clear, step-by-step format showing your reasoning process.`;
  ...
  async generateRiskScenarioAnalysis(
    portfolio: any,
    marketScenarios: any[]
  ): Promise<AIProviderResponse> {
